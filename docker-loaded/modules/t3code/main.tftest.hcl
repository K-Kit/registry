...
run "defaults_are_secure" {
  ...
  assert {
    condition     = strcontains(nonsensitive(local.start_script), "--host 127.0.0.1")
    error_message = "The startup script must bind T3 Code to IPv4 loopback."
  }

  assert {
    condition     = !strcontains(nonsensitive(local.start_script), "0.0.0.0")
    error_message = "The startup script must not expose T3 Code on all network interfaces."
  }

  assert {
    condition     = strcontains(nonsensitive(local.start_script), "--auto-bootstrap-project-from-cwd")
  ...
  assert {
    condition     = strcontains(nonsensitive(local.install_script), "\"allowScripts\"") && strcontains(nonsensitive(local.install_script), "\"node-pty\":true") && strcontains(nonsensitive(local.install_script), "\"msgpackr-extract\":true")
    error_message = "The install script must approve T3 Code's native dependency install scripts for npm 12."
  }

  assert {
    condition     = strcontains(nonsensitive(local.install_script), "K_KIT_RELEASES_URL=\"https://github.com/K-Kit/t3code/releases\"") && strcontains(nonsensitive(local.install_script), "PACKAGE_DIR=\"$RUNTIME_DIR/node_modules/@k-kit/t3code\"")
    error_message = "latest must resolve to the public K-Kit package containing the OMP/Pi fix."
  }

  assert {
    condition     = strcontains(nonsensitive(local.install_script), "[ \"$current_version\" = \"$DESIRED_VERSION\" ]") && !strcontains(nonsensitive(local.install_script), "[ \"$ARG_T3CODE_VERSION\" = \"latest\" ] ||")
    error_message = "latest must skip installation only when the installed and resolved versions match."
  }

  assert {
    condition     = strcontains(nonsensitive(local.install_script), "--allow-remote=root")
...
run "custom_configuration" {
  ...
  assert {
    condition     = strcontains(nonsensitive(local.start_script), base64encode("/home/coder/project"))
    error_message = "The configured working directory must be encoded into the startup script."
  }

  assert {
    condition     = strcontains(nonsensitive(local.start_script), "ARG_AUTO_BOOTSTRAP_PROJECT='false'")
    error_message = "The disabled project bootstrapping setting must be rendered into the startup script."
  }

  assert {
    condition     = strcontains(nonsensitive(local.start_script), base64encode("test-pairing-secret"))
    error_message = "The pairing secret must be encoded into the startup script."
  }

  assert {
    condition     = !strcontains(nonsensitive(local.start_script), "ARG_PAIRING_SECRET_B64='test-pairing-secret'")
    error_message = "The pairing secret must not be rendered into the startup script as plaintext."
  }

  assert {
    condition     = strcontains(nonsensitive(local.start_script), "--bootstrap-fd 3")
    error_message = "A configured pairing secret must be passed through T3 Code's bootstrap descriptor."
  }
}
...
run "rejects_short_pairing_secret" {
  command = plan

  variables {
    agent_id       = "test-agent"
    pairing_secret = "too-short"
  }
