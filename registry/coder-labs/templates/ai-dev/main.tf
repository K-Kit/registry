...
data "coder_parameter" "ai_agents" {
  ...
  default      = jsonencode(["Claude Code", "Codex", "OpenCode"])
  mutable      = false
  order        = 2
...
locals {
  ...
  t3code_start_script = templatefile("${path.module}/scripts/t3code-start.sh.tftpl", {
    ...
    ARG_PAIRING_PROXY_B64      = base64encode(file("${path.module}/scripts/t3code-pairing-proxy.mjs"))
    ARG_PAIRING_PROXY_PORT     = "3774"
    ARG_PORT                   = "3773"
...
resource "coder_app" "t3code" {
  ...
  url          = "http://localhost:3774/__coder_pair"
  icon         = "/icon/t3code.svg"
  subdomain    = true
  share        = "owner"
  ...
  healthcheck {
    url       = "http://localhost:3774/__health"
    interval  = 5
    threshold = 12
  }
}
