...
variable "pairing_secret" {
  description = "Optional reusable pairing secret for T3 Code clients. Leave empty to use T3 Code-generated pairing tokens."
  type        = string
  default     = ""

  validation {
    condition     = var.pairing_secret == "" || length(var.pairing_secret) >= 12
    error_message = "pairing_secret must be empty or at least 12 characters."
  }
...
locals {
  ...
  start_script = templatefile("${path.module}/scripts/start.sh.tftpl", {
    ...
    ARG_PAIRING_SECRET_B64     = base64encode(var.pairing_secret)
